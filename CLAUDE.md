...
| **(v3.1.0) VEO render fails `Audio generation failed`** | The prompt's audio line is a negated list (`no music, no voices`). Rewrite it as a positive ambience description and retry; the render offer retries once automatically for this exact error. |
...
| **(v3.1.0) P6 (`verify_render.py`) reports missing or inserted words** | The rendered master says something different from `av-script.md`. Listen at the timestamps printed in `work/verify-report.md`; fix the edit-plan segment or regenerate the VO for that layer — never edit the report to make it pass. |
